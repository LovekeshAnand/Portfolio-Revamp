    ...
    <div className="relative min-h-screen bg-[#070707] text-white flex flex-col select-none overflow-hidden pt-32">
      ...
      <main className="flex-1 relative z-10 max-w-7xl mx-auto px-6 md:px-12 w-full pb-24">
        ...
        <div className="mb-14">
          ...
          <p className="font-author text-sm text-neutral-400 font-light mt-4 max-w-xl leading-relaxed">
        ...
        {/* Tab Filters */}
        <div className="flex flex-wrap items-center gap-2 md:gap-3 border-b border-white/5 pb-6 mb-10 font-mono text-[9px] md:text-xs">
          {categories.map((cat) => (
            <button
              key={cat.id}
          ...
          {filteredProjects.map((project) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 pointer-events-none select-none">
                <span className="text-[7.5px] font-mono font-bold text-neutral-500 tracking-[0.2em] uppercase">
                  {project.category} // DISPATCH {project.id.slice(0, 3)}
                </span>
                <span className="text-[8px] font-mono text-orange-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  EXPLORE ↗
                </span>
              </div>
              ...
              <div className="mb-2 select-none pointer-events-none">
                ...
                <p className="text-orange-400 font-mono text-[9px] uppercase tracking-wider mt-1">
                  {project.role}
                </p>
              </div>

              {/* Description */}
              <p className="font-author text-xs text-neutral-400 leading-relaxed font-light mb-4 select-text">
                {project.description}
              </p>
              ...
              <div className="flex flex-wrap gap-1 mb-4 select-none pointer-events-none">
                {project.tech.map((s) => (
                  <span
                    key={s}
                    className="text-[8px] font-mono px-2 py-[2px] rounded border border-white/5 bg-white/[0.01] text-neutral-500 group-hover:border-orange-500/10 group-hover:text-neutral-300 transition-colors duration-300"
                  >
                    {s}
                  </span>
                ))}
              ...
              {project.metrics && (
                <div className="border-t border-white/[0.04] pt-3 flex justify-between font-mono text-[8px] text-neutral-600 select-none pointer-events-none">
                  {project.metrics.map((metric) => (
                    <div key={metric.label} className="flex flex-col">
                      <span className="text-neutral-500/70 text-[6.5px] tracking-wider">{metric.label}</span>
                      <span className="text-neutral-400 group-hover:text-orange-400 transition-colors duration-300 mt-0.5">{metric.value}</span>
                    </div>
                  ))}
                </div>
              )}
